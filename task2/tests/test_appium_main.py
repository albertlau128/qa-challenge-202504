...
class TestAppium(unittest.TestCase):
    def setUp(self) -> None:
        ...
        if self.driver.current_activity=='.AgreementPage':
            ...
            for _ in range(2): 
                el.click()
            # time.sleep(2)
            print(f"Current app activity: {self.driver.current_activity}, expected `.myObservatory_app_SplashScreen`")
            self.driver.find_element(by=AppiumBy.XPATH, value='//android.widget.Button[@text="OK"]').click()
            # swipe left
            time.sleep(10)
            swipe_left(self.driver, start_x=0.9, stop_x=0.1)

            time.sleep(1)
            not_show_again = self.driver.find_elements(by=AppiumBy.XPATH, value='//*[@text="Do not show again"]')
            if not_show_again:
    ...
    def test_hko_get_forecast_next_date(self) -> None:
        ...
        while self.driver.current_activity!='.WeatherForecastActivity':
            ...
            self.driver.find_element(by=AppiumBy.XPATH, value='//android.widget.ImageButton[@content-desc="Navigate up"]').click()
        
            self.driver.find_element(by=AppiumBy.XPATH, value='//*[@text="Forecast & Warning Services"]').click()
            ...
            self.driver.find_elements(by=AppiumBy.XPATH, value='//android.widget.ImageView[@resource-id="hko.MyObservatory_v1_0:id/icon"]')[4].click()
            print(f"Current app activity: {self.driver.current_activity}, expected `.WeatherForecastActivity`")
        ...
        cards_top = self.driver.find_elements(by=AppiumBy.XPATH, value="//android.widget.LinearLayout")[7:15]
        for c in cards_top:
